"""Performance measurement primitives (Task 5.4 / building blocks).

Captures every metric the assignment asks for, in raw form so figures can be
regenerated later:
  - TTFT  (Time To First Token)         -> prefill cost / KV-cache build
  - ITL / TPOT (Time Per Output Token)  -> decode cost / memory-bound stream
  - Throughput (tokens/sec)
  - Peak RAM and peak VRAM
  - Wall-clock time and an estimated energy figure

Memory is sampled on a background thread so we catch the *peak*, not just the
end state. All numbers are stored raw (results/*.json).
"""
from __future__ import annotations

import json
import threading
import time
from dataclasses import asdict, dataclass, field

import psutil


@dataclass
class RunMetrics:
    """One measured (or failed) run.

    Input:  label/model/quantization identify the run; counters are filled in.
    Output: a JSON-serialisable record (save) and derived TPOT/throughput.
    """

    label: str
    model: str
    quantization: str
    prompt: str = ""               # which prompt was run (short/medium/long_context)
    prompt_tokens: int = 0
    output_tokens: int = 0
    ttft_s: float = 0.0
    tpot_s: float = 0.0
    total_gen_s: float = 0.0
    throughput_tok_s: float = 0.0
    peak_ram_gb: float = 0.0
    peak_vram_gb: float = 0.0
    est_energy_wh: float = 0.0
    failed: bool = False
    expected_failure: bool = False   # True when not-completing IS the intended result
    failure_reason: str = ""
    per_token_latencies_s: list[float] = field(default_factory=list)

    def finalize(self) -> None:
        """Derive TPOT and throughput from the raw counters."""
        decode_time = max(self.total_gen_s - self.ttft_s, 1e-9)
        self.tpot_s = decode_time / max(self.output_tokens - 1, 1)
        self.throughput_tok_s = self.output_tokens / max(self.total_gen_s, 1e-9)

    def save(self, path: str) -> None:
        """Persist the raw record as indented JSON."""
        with open(path, "w", encoding="utf-8") as handle:
            json.dump(asdict(self), handle, indent=2)


class MemorySampler:
    """Polls RSS (and VRAM if torch+cuda available) on a background thread.

    Input:  interval_s (poll period), avg_power_w (for the energy estimate).
    Output: peak_ram_gb / peak_vram_gb / energy_wh after the `with` block.
    """

    def __init__(self, interval_s: float = 0.1, avg_power_w: float = 65.0) -> None:
        self.interval_s = interval_s
        self.avg_power_w = avg_power_w
        self._stop = threading.Event()
        self._thread: threading.Thread | None = None
        self.peak_ram_gb = 0.0
        self.peak_vram_gb = 0.0
        self._proc = psutil.Process()
        self._t0 = 0.0
        self._elapsed = 0.0
        self._torch = None
        try:
            import torch

            if torch.cuda.is_available():
                self._torch = torch
                torch.cuda.reset_peak_memory_stats()
        except ImportError:
            pass

    def _loop(self) -> None:
        while not self._stop.is_set():
            rss = self._proc.memory_info().rss / (1024**3)
            self.peak_ram_gb = max(self.peak_ram_gb, rss)
            if self._torch is not None:
                vram = self._torch.cuda.max_memory_allocated() / (1024**3)
                self.peak_vram_gb = max(self.peak_vram_gb, vram)
            time.sleep(self.interval_s)

    def __enter__(self) -> MemorySampler:
        self._t0 = time.perf_counter()
        self._stop.clear()
        self._thread = threading.Thread(target=self._loop, daemon=True)
        self._thread.start()
        return self

    def __exit__(self, *exc: object) -> None:
        self._stop.set()
        if self._thread:
            self._thread.join()
        self._elapsed = time.perf_counter() - self._t0

    @property
    def energy_wh(self) -> float:
        """Coarse estimate: average package power * wall time."""
        return self.avg_power_w * (self._elapsed / 3600.0)


def time_streaming_generation(token_iter, metrics: RunMetrics) -> str:
    """Consume a streaming token iterator, recording TTFT and per-token gaps."""
    pieces: list[str] = []
    start = time.perf_counter()
    last = start
    first_seen = False
    for chunk in token_iter:
        now = time.perf_counter()
        if not first_seen:
            metrics.ttft_s = now - start
            first_seen = True
        else:
            metrics.per_token_latencies_s.append(now - last)
        last = now
        pieces.append(chunk)
        metrics.output_tokens += 1
    metrics.total_gen_s = time.perf_counter() - start
    metrics.finalize()
    return "".join(pieces)
